...
export default function ClientLoginScreen() {
  ...
  // Auto-prompt Face ID if credentials saved
  useEffect(() => {
    if (bio.available && bio.hasSavedCredentials) handleBiometricLogin();
  ...
  const handleBiometricLogin = async () => {
    ...
    try {
      ...
      if (!creds) return;
      const res = await ClientApi.login({ email: creds.email, password: creds.password });
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (bio.available && !bio.hasSavedCredentials) {
        Alert.alert(`Enable ${bio.biometricLabel}?`, `Sign in faster next time with ${bio.biometricLabel}.`, [
          { text: 'Not now', style: 'cancel', onPress: () => router.replace('/(consumer-tabs)/discover') },
          { text: 'Enable', onPress: async () => { await bio.saveCredentials(email.trim().toLowerCase(), password); router.replace('/(consumer-tabs)/discover'); } },
        ]);
      } else {
        router.replace('/(consumer-tabs)/discover');
      }
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={[styles.scroll, { paddingTop: insets.top + 20, paddingBottom: insets.bottom + 24 }]}>
        ...
        {bio.available && bio.hasSavedCredentials && (
          <TouchableOpacity style={[styles.bioBtn, bioLoading && { opacity: 0.6 }]} onPress={handleBiometricLogin} disabled={bioLoading}>
            {bioLoading ? <ActivityIndicator color={Colors.rose} size="small" /> : <Text style={styles.bioBtnText}>🔐 Sign in with {bio.biometricLabel}</Text>}
          </TouchableOpacity>
        )}
        {google.ready && (
