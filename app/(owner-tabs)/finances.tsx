...
export default function FinancesScreen() {
  ...
  return (
    <View style={[s.container, { paddingTop: insets.top }]}>
      ...
      >
        {loading ? <ActivityIndicator color={C.rose} size="large" style={{marginTop:40}} /> : (<>

        {/* Revenue stats */}
        <Text style={s.sectionTitle}>Revenue Overview</Text>
        <View style={s.statsGrid}>
          <StatCard label="This Week"  value={fmt(rev.week||0)}  accent={C.rose} />
          ...
          <>
            ...
            <View style={s.card}>
              {byService.slice(0,8).map((sv,i) => (
                <View key={i} style={[s.svcRow, i>0 && { borderTopWidth:1, borderTopColor:C.border }]}>
        ...
        {filteredBookings.length === 0 ? (
          ...
          const stCol    = STATUS_COLOR[status]||C.soft;
          return (
            <View key={b.id||i} style={s.txRow}>
              ...
              <View style={{alignItems:'flex-end',gap:4}}>
                <Text style={[s.txPrice, price>0 && { color:C.charcoal }]}>{price>0?fmt(price):'—'}</Text>
...
const s = StyleSheet.create({
  ...
  header:        { paddingHorizontal:20, paddingVertical:16, borderBottomWidth:1, borderBottomColor:C.border, backgroundColor:C.cream },
  heading:       { fontSize:22, fontWeight:'900', color:C.charcoal, fontFamily:'Georgia' },
  scroll:        { padding:14 },
  sectionTitle:  { fontSize:11, fontWeight:'800', color:C.rose, textTransform:'uppercase', letterSpacing:1, marginTop:20, marginBottom:10, paddingHorizontal:2 },
  statsGrid:     { flexDirection:'row', gap:8, marginBottom:4 },
  statCard:      { flex:1, backgroundColor:C.white, borderRadius:14, padding:14, alignItems:'center', borderWidth:1, borderColor:C.border, shadowColor:C.charcoal, shadowOffset:{width:0,height:1}, shadowOpacity:0.05, shadowRadius:4, elevation:1 },
  statVal:       { fontSize:18, fontWeight:'900', color:C.charcoal, marginBottom:4 },
  statLabel:     { fontSize:9, fontWeight:'700', color:C.soft, textAlign:'center', textTransform:'uppercase', letterSpacing:0.3 },
  statSub:       { fontSize:9, color:C.rose, marginTop:2, fontWeight:'600' },
  periodBar:     { flexDirection:'row', backgroundColor:C.white, borderRadius:12, borderWidth:1, borderColor:C.border, padding:3, gap:2 },
  periodBtn:     { flex:1, paddingVertical:8, borderRadius:9, alignItems:'center' },
  periodBtnOn:   { backgroundColor:C.rose },
  periodBtnTxt:  { fontSize:12, fontWeight:'700', color:C.soft },
  periodBtnTxtOn:{ color:C.white, fontWeight:'800' },
  periodSummary: { flexDirection:'row', backgroundColor:C.white, borderRadius:14, borderWidth:1, borderColor:C.border, padding:16, marginTop:10, alignItems:'center' },
  summaryItem:   { flex:1, alignItems:'center' },
  summaryVal:    { fontSize:16, fontWeight:'900', color:C.charcoal },
  summaryKey:    { fontSize:10, color:C.soft, fontWeight:'600', marginTop:2 },
  summaryDivider:{ width:1, height:32, backgroundColor:C.border },
  card:          { backgroundColor:C.white, borderRadius:14, borderWidth:1, borderColor:C.border, overflow:'hidden', marginTop:4 },
  svcRow:        { flexDirection:'row', justifyContent:'space-between', alignItems:'center', padding:14 },
  svcName:       { fontSize:13, fontWeight:'700', color:C.charcoal, flex:1 },
  svcRevenue:    { fontSize:14, fontWeight:'800', color:C.charcoal },
  svcCount:      { fontSize:10, color:C.soft, marginTop:2 },
  txRow:         { flexDirection:'row', alignItems:'flex-start', backgroundColor:C.white, borderRadius:12, padding:14, marginBottom:8, borderWidth:1, borderColor:C.border },
  txClient:      { fontSize:14, fontWeight:'700', color:C.charcoal },
  txService:     { fontSize:12, color:C.soft, marginTop:2 },
  txDate:        { fontSize:11, color:C.mid, marginTop:3 },
  txPrice:       { fontSize:16, fontWeight:'900', color:C.soft },
